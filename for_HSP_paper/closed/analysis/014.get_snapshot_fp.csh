...
foreach rst ($list)
   ...
   cut -c 17-27 ${rst:r}.pdb | uniq | grep -v LIG  | sed -e 's/ //g' | head -468 > lab1.txt 
   cut -c 17-27 ${rst:r}.pdb | uniq | grep -v LIG  | sed -e 's/ //g' | head -966 | tail -1 >> lab1.txt 

   cut -c 17-27 ${rst:r}.pdb | uniq | grep -v LIG  | sed -e 's/ //g' | head -965 | tail -497 > lab2.txt 
   cut -c 17-27 ${rst:r}.pdb | uniq | grep -v LIG  | sed -e 's/ //g' | head -967 | tail -1 >> lab2.txt 

   mv vdw${rst:t:r}_fp.avg vdw${rst:t:r}_fp.avg.txt
   mv ele${rst:t:r}_fp.avg ele${rst:t:r}_fp.avg.txt
 
   python ${script}/examples_analysis/heatmap_matrix_mod.py vdw${rst:t:r}_fp.avg.txt 0.0 -1.0 1.0 lab1.txt lab2.txt >  ${rst:t:r}_heatmap_matrix.VDW.log
   python ${script}/examples_analysis/heatmap_matrix_mod.py ele${rst:t:r}_fp.avg.txt 0.0 -50.0 50.0 lab1.txt lab2.txt >  ${rst:t:r}_heatmap_matrix.ES.log
   #exit
end # rst
end # seed
