...
def main():
   ...
   #    w: white
    ...
    for i,desc in enumerate(data):
       ...
       n, b, p = axis.hist(desc, density=True, histtype='bar')
    ...
    subpanel = [ [0.2,0.1,0.3,0.2], [0.65,0.1,0.3,0.2], [0.2,0.4,0.3,0.2], [0.6,0.4,0.3,0.2], [0.2,0.7,0.3,0.2], [0.6,0.7,0.3,0.2] ]
    fig = pylab.figure(figsize=(8,8))
    ...
    time_ns = [int(val/100) for val in time]
    im = axis.plot(time_ns,el,'k-') #,[0,100],[0,100],'--')
    axis.set_xlabel("time (ns)")
    for tick in axis.get_xticklabels():
      tick.set_rotation(45)
    ...
    subpanel = [ [0.2,0.1,0.3,0.2], [0.6,0.1,0.3,0.2], [0.2,0.4,0.3,0.2], [0.6,0.4,0.3,0.2], [0.2,0.7,0.3,0.2], [0.6,0.7,0.3,0.2] ]
    print("ploting %s"%header[4])
    name = header[4]
    bindE = data[4]
    time_t = range(1,len(bindE)+1)
    time = [int(val/100) for val in time_t]
    fig = pylab.figure(figsize=(8,8))
    axis = fig.add_axes(subpanel[0])
    axis.plot(time,bindE,'k-') #,[0,100],[0,100],'--')
    sw_time,sw_bindE = conv.sliding_wendow(time,bindE,100)
    axis.plot(sw_time,sw_bindE,'g-') #,[0,100],[0,100],'--')
    axis.set_xlabel("time (ns)")
    axis.set_ylabel(name+"(kcal/mol)")
    axis.set_title("Sliding window")
    acf_bindE = conv.ACF(bindE,1)
    axis = fig.add_axes(subpanel[1])
    axis.plot(acf_bindE,'k-')
    axis.set_title("Autocorrelation function")
    blocks_bindE = conv.BASEM(bindE,100)
    axis = fig.add_axes(subpanel[2])
    axis.plot(blocks_bindE,'k-')
    axis.set_title("Block averages")
    axis = fig.add_axes(subpanel[3])
    m,b,bindEfit = conv.fit_line(time,bindE)
    axis.plot(time,bindE,'k.',time,bindEfit,'g-')
    axis.text(0.95, 0.01, 'y = %f x + %f'%(m,b),
        verticalalignment='bottom', horizontalalignment='right',
        transform=axis.transAxes,
        #color='green', fontsize=15)
        color='green', fontsize=8)
    axis.set_title("Fit line")
    #fig.savefig(filenamepng.replace('.png','')+"_converge.png",dpi=600)
    axis = fig.add_axes(subpanel[4])
    sarray,basemarray = conv.BASEM_vs_Blocksize(bindE)
    axis.plot(sarray,basemarray,'k-')
    axis.set_xlabel("block size (# of snapshots)")
    axis.set_ylabel("BASEM (kcal/mol)")
    fig.savefig(filenamepng+"_converge.png",dpi=600)
