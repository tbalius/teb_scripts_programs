...
set workdir  = $mountdir/${pdb}/014.footprint_single_point_alt/name_${name}/

rm -rf $workdir
mkdir -p $workdir
cd $workdir

echo "${mountdir}/../Closed_alt/0004.min/*.rst7"
ls ${mountdir}/../Closed_alt/0004.min/*.rst7

set list = `ls ${mountdir}/../Closed_alt/0004.min/*.rst7`
set prm  =  ${mountdir}/../Closed_alt/0003md_tleap/com1.mmrc.leap.prm7 
...
foreach rst ($list)
   ...
   set rstname = $rst:t
   #exit
   if (${rst:t} == '01mi.rst7') then
cp $rst $rstname.ori
cat << EOF > cpptraj.in
trajin ${rstname}.ori 
trajout ${rstname} restart
EOF

   /home/${username}/zzz.programs/amber/amber18/bin/cpptraj -p $prm -i cpptraj.in -o cpptraj.out 
   else
   cp $rst .
   endif
