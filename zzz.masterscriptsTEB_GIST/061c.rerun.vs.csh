...
# This script runs the VS of a specified database 
 ...
 # for natural products (run in SF and copies over to UofT) the path is:
  set docktype =  "_flex__nogist"
 #  set docktype =  "_min"
 # set docktype =  "_flex__gist"
## set prefixName = "frag"
## set indexfile = ../020.get_database_index_files/2017.03.07.ZINC-downloader-3D-db2.gz.database_index

## set desiredDirectoryCount = "1000"	# especially for leads, for other ones <2000 suffices

 set mountdir = `pwd`
 echo "mountdir = $mountdir"
## set filedir = $mountdir/docking/1prep/  # CHANGE THIS

 #set pdbname =  "3AZ2"
 set workdir = "$mountdir/vs${docktype}" 
