...
# cp /mnt/nfs/work/rstein/Big_Blurry/calc_rmsd.py 017b.calc_rmsd.py
...
def main():
	...
	blurry_direc = pwd+pdb+"_blurry_spheres/"

	os.chdir(blurry_direc)
	
	#sys_dict = {pdb+"_blurry_pose_RMSD":[pdb+'_GIST_0p5']}
	sys_dict = {pdb+"_blurry_pose_RMSD":[pdb+'_sig_2_gist_rad_1pt8_1pt0_0pt5x']}

	#dir_list = [name for name in os.listdir(".") if os.path.isdir(name) and name[0:4] == "1L2S" and name[-1] != "0"]
	#dir_list = [name for name in os.listdir(".") if os.path.isdir(name) and name[-30:] == "sig_2_gist_rad_1pt8_1pt0_0pt5x"]
	dir_list = [name for name in os.listdir(".") if os.path.isdir(name) and name[-34:] == "sig_2_gist_rad_1pt8_1pt0_0pt5x_min"]

	for direc in dir_list:
		sys_dict[pdb+"_blurry_pose_RMSD"].append(direc)
	print(sys_dict)
	...
	for key in sys_dict:	
		...
		standard = pwd+pdb+"_blurry_spheres/"+sys_dict[key][0]
		gist_system = sys_dict[key][1:]
		new_dir1 = blurry_direc+dir_name
		...
		for gtype in gist_system:
			system_name = gtype 
			print(system_name)
			new_dir2 = blurry_direc+dir_name+"/RMSD_"+gtype
			...
			standard_poses = standard+"/ligands-decoys/ligands/allChunksCombined/poses.mol2"
			gist_poses = blurry_direc+gtype+"/ligands-decoys/ligands/allChunksCombined/poses.mol2"
			...
			for lig in lig_list:
				...
				for dup in duplicates:		
					...
					output_rmsd_file = dup.strip().split(".")[0]+"_mod.mol2"
					output_rmsd = open(output_rmsd_file,"w")
					write_out(mol2_list,output_rmsd)
					output_rmsd.close()
				for i in range(len(dup_list)):
					dup_list_mod.append(dup_list[i].strip().split(".")[0]+"_mod.mol2")
