...
# written in Dec, 2016
# This was modifed for calculating blurry gist weighting at FNLCR in 2020


def bounding_box_grid_file(sphs,scale,pad):
  ...
  buff = scale + pad # add/substract a buffer to make sure spheres are completly contained
...
def calc_weights(fileprefix,values,gridscale,xn,yn,zn,origin,sphs,sigma,fileh,return_vals,pad):
# file - log file
# values gist values 
            ...
            for j in range(int(start_j),int(stop_j)):
                ...
                for k in range(int(start_k),int(stop_k)):
                    ...
                    if (dist <= radius):
                        ...
                        weight = (1.0/math.sqrt(2.0 * math.pi * sigma**2.0)) * math.exp(-1.0*(dist**2.0)/(2.0 * (sigma**2)))
                        #new_values.append(weight)
                        new_values[count] = new_values[count] + weight 
                    #else:
                    #    new_values.append(0.0)
...
def main():
   ...
   if len(sys.argv) != 5: # if no input
       print "ERORR:"
       print "syntex: burry_gist_weight_cal_for_fig.py sphere_file scale sigm aoutputprefix "
       ...
       print "sigma dinfines the fines width of the pick about the atom."
       return
   ...
   xn,yn,zn,dx,dy,dz,origin,values = bounding_box_grid_file(sphs,scale,pad)

   gridscale = dx # assumes that they are all the same spaceing
   
   print gridscale ,xn,yn,zn,dx,dy,dz,origin   

   file1 = open(outfile+'weight_values.txt','w')

   count = 0
   new_values = calc_weights(outfile,values,gridscale,xn,yn,zn,origin, sphs, sigma, file1, True, pad)
