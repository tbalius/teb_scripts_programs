...
#################################################################################################################
#################################################################################################################
def read_TOPDOCK_PDB_file(file):
    # reads in data from multi-Mol2 file.
    ...
    for line in lines:
         ...
         if (linesplit[0] == "REMARK" and linesplit[1] == "Name"):
             Name = linesplit[3]
             #line_num = line_num + 1

## column 11 is the type in the vdw parameter file ($DOCK_BASE/data/vdw.parms.amb)
## we could write a function that will read in the vdw.parms.amb and will look up type 
## and cross reference with sybyl atom types

##ATOM      1  O1  AAA     1      21.275   4.849  46.760  -0.315   -1.96 12
##ATOM      2  C2  AAA     1      22.872   5.975  52.042   0.095    0.10  5

         if ((len(linesplit) >= 10 )and (flag_atom)):
             #print "I AM HERE"
             atom_num  = linesplit[1]
             atom_name = linesplit[2]
             X         = linesplit[5]
             Y         = linesplit[6]
             Z         = linesplit[7]
             ## ($DOCK_BASE/data/vdw.parms.amb) to sybyl 
             if (int(linesplit[10]) == 6 or int(linesplit[10]) == 7):
                #print "I AM HERE"
                atom_type = "H"
             else:
                atom_type = "  "
             res_num   = int(linesplit[4])
             res_name  = linesplit[3]
             Q         = linesplit[8]
             temp_atom = atom(X,Y,Z,Q,atom_type,atom_name,atom_num,res_num,res_name)
             atom_list.append(temp_atom)
	     if residue_list.has_key(res_num):
