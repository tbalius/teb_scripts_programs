...
if len(sys.argv) != 6:
   print ("error:  this program takes 2 input extract filename. 2 lables and a thresshold value for heatmaps   ")
   exit()
...
for i in range(0,10):
    label = axis.xaxis.get_major_ticks()[i].label1
...
for item in (axis.xaxis.get_major_ticks()):
    ...
    item.label1.set_rotation('vertical')
...
## new subplot
axis = fig2.add_axes([0.6,0.15,0.3,0.3])
Xmax = max(Xscore)
Xmin = min(Xscore)
Ymax = max(Yscore)
Ymin = min(Yscore)
...
axis = fig3.add_axes([0.1,0.1,0.3,0.2])
...
#print len(n1)
#print len(bins1)
axis = fig3.add_axes([0.1,0.4,0.3,0.2])
im = axis.plot(midbin1, n1,'r-') #,[0,100],[0,100],'--')
#panal
axis = fig3.add_axes([0.5,0.1,0.3,0.2])
...
#print len(n2)
#print len(bins2)
axis = fig3.add_axes([0.5,0.4,0.3,0.2])
im = axis.plot(midbin2, n2,'b-') #,[0,100],[0,100],'--')

axis = fig3.add_axes([0.5,0.7,0.3,0.2])
im = axis.plot(midbin1, n1,'r-',midbin2, n2,'b-') #,[0,100],[0,100],'--')

fig3.show()
fig3.savefig('fig3.png',dpi=600)

#fig4 = pylab.figure(figsize=(8,8))
#
#ymax = max(Y_top)
#xmax = max(X_top)
#
#axis = fig4.add_axes([0.1,0.1,0.3,0.3])
...
#
#axis = fig4.add_axes([0.5,0.1,0.3,0.3])
#im = axis.plot(X_top,Y_top,'o') #,[0,100],[0,100],'--')
...
#axis.set_yticks([0, 500 ,1000])
#
#axis = fig4.add_axes([0.1,0.5,0.3,0.3])
#im = axis.plot(X_top,Y_top,'o') #,[0,100],[0,100],'--')
#axis.set_xlim( 0, 1000)
