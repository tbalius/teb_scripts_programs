...
def min_dist_between_two_sets_of_atoms(atom_list1,atom_list2,filename2,threshold):
        dict_res = {}
        flag_write = False
        for i in range(0,len(atom_list1)):
           for j in range(0,len(atom_list2)):
               ...
               if (dist2<threshold**2.0): 
                   ...
                   res = filename2+"::"+atom_list2[j].resname+atom_list2[j].resnum
...
def main():
    if len(sys.argv) != 5: # if no input
       print ("ERORR: there need to be 4 inputs: pdb file_pdb_list lig_resid dcutoff (center of ligand to center of model)")
       return
    ...
    for i,center in enumerate(center_list):
        ...
        if d < ligand_dist: 
           print("The model within cutoff (dist = %f) to center of ligand is %s" %(d,pdb_f_list[i]))
    ...
    for filename in filelist: 
       ...
       flag_write = min_dist_between_two_sets_of_atoms(atom_lig_list,atom_list,filename,5)
       if flag_write: 
