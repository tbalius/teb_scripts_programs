...
def cluster_points(points):
    ...
    for i in range(N):
        ...
        for j in range(i+1,N):
            ...
            ind_j2 = points[j].J 
            ind_k2 = points[j].K 
            if math.fabs(ind_i2-ind_i1) + math.fabs(ind_j2-ind_j1) + math.fabs(ind_k2-ind_k1) == 1: # if 0, they are the same, if grater than one they are not connected.  
               bond = [i, j]
               bonds.append(bond)
               bonded_p[i] = 1
               bonded_p[j] = 1
...
def make_centers(fileprefix,values,gridscale,xn,yn,zn,origin,sign,threshold):
# file - log file
# values gist values 
...
#
    ...
    clusters, count = cluster_points(points) 
...
def main():
   ...
   if len(sys.argv) != 5: # if no input
       print "ERORR:"
       print "syntex: dx-gist_make_centers.py dx-input-file sign cutoff pdb-output-file"
   ...
   make_centers(outfile,values,gridscale,xn,yn,zn,origin, sign, threshold)
