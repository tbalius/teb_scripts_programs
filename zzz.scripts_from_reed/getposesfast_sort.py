...
def write_out_poses(lig_dict, lig_dir):
	...
	for name in lig_dict:
		energy = lig_dict[name][1]
		list_lig.append([name, energy])
	#list_lig_sort = sorted(list_lig, key=lambda ele: (-1.0*ele[1]))   # sort by energy
	list_lig_sort = sorted(list_lig, key=lambda ele: ele[1])   # sort by energy
        #print list_lig_sort

	output = open("poses.mol2",'w')
	#for name in lig_dict:
	for name,energy in list_lig_sort:
		...
		db2_gz_file = lig_dir+dirname+"/test.mol2.gz" 
		...
		for new_line in ligandFile:
			...
			if new_line_count == line_num:  # line is in the list so set flag to True for writting out
				#print(new_line)
				if len(splitline) > 1:
					if splitline[1] == "Name:":
						name_found = True	
				...
				if (len(splitline) > 1):
					...
					if splitline[1] == "Name:" and name_found and new_line_count != line_num:
						name_found = False
