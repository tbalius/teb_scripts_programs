...
def fingerprint(SmilesString,pid):
    ...
    fh = open("/tmp/"+ name +"/temp"+pid+".smi",'w')
    ...
    comand = Generatemd + " c /tmp/" + name + "/temp"+pid+".smi -k ECFP -2"
    print "runing the comand:"+comand
    output = os.popen(comand).readlines()
    #print "output:"+str(output)
    fp = output[0].strip('\n')
    # remove the temp file. 
    os.system("rm -fr "+"/tmp/" + name + "/temp"+pid+".smi")
    #print fp
    return fp
...
def molecularMass(SmilesString,pid):
    ...
    comand = Generatemd + " c /tmp/" + name + "/temp"+pid+".smi -k Mass"
    print "runing the comand:"+comand
    output = os.popen(comand).readlines()
    #print output
    #print "output:"+str(output)
...
def heavyAtoms(SmilesString,pid):
    ...
    comand = Generatemd + " c /tmp/" + name + "/temp"+pid+".smi -k Heavy"
    print "runing the comand:"+comand
    output = os.popen(comand).readlines()
    #print "output:"+str(output)
    #outlines = output.split('\n')
