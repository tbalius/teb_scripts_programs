...
def main():
  ...
  for j in range(len(int_list1)):
      avg_fp_int.append(0.0)
      avg_fp_vdw.append(0.0)
      avg_fp_ele.append(0.0)
  fileh = open(output,'w')
  for i in range(len(frames)):
    ...
    for resid1 in int_list1:
       ...
       avg_fp_int[j] = avg_fp_int[j]+Eint
       avg_fp_vdw[j] = avg_fp_vdw[j]+Evdw
       avg_fp_ele[j] = avg_fp_ele[j]+Ees
       print resid1,Eint,Evdw,Ees
       fileh.write('%d,%f,%f,%f\n'%(resid1,Eint,Evdw,Ees))
       j=j+1
  fileh.write("AVG fp\n")

  for j in range(len(int_list1)):
    avg_fp_int[j] = avg_fp_int[j]/len(frames)
    avg_fp_vdw[j] = avg_fp_vdw[j]/len(frames)
    avg_fp_ele[j] = avg_fp_ele[j]/len(frames)
