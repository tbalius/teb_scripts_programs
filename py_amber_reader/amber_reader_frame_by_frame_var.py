...
def coord_reader(line_count_ori,fh,frame1,filename,size,remainder):
 
 more_cord_flag = False
 flag_break = False
...
def intermolecular_Energy(parm_stuff,frameX,start1,stop1,start2,stop2):
  ...
  # #for debugging stop
    ...
    for i in range(start1,stop1):
        for j in range(start2,stop2):
             ...
             r = distance(frameX.cords[i-1],frameX.cords[j-1])
...
def main():
  ...
  for j in range(len(int_list1)):
      ...
      for k in range(len(int_list2)):
          ...
          row2.append(0.0)
          row3.append(0.0)
      avg2_mat_int.append(row1)
      avg2_mat_vdw.append(row2)
      avg2_mat_ele.append(row3)
  ...
  for j in range(len(int_list1)):
      ...
      for k in range(len(int_list2)):
          ...
          row2.append(0.0)
          row3.append(0.0)
      var_mat_int.append(row1)
      var_mat_vdw.append(row2)
      var_mat_ele.append(row3)

  frameL = []
  remainder_data = [] # this will store the remaining cordenates at the end of the line.  
  i = 0 
  fh = open(crdfile,'r')
  ...
  #for i in range(len(frames)):
    ...
    #    exit()
    print ('remainder: ',len(remainder_data) )
    frameX = frameL[0]
    del frameL[:]
    j = 0
    if oflag:
    ...
    for resid1 in int_list1:
       ...
       for resid2 in int_list2:
         ...
         avg2_mat_int[j][k] = avg2_mat_int[j][k]+Eint**2.0
         avg2_mat_vdw[j][k] = avg2_mat_vdw[j][k]+Evdw**2.0
         avg2_mat_ele[j][k] = avg2_mat_ele[j][k]+Ees**2.0
  ...
  varvdwfileh = open("vdw"+output+".avg",'w')
  varelefileh = open("ele"+output+".avg",'w')
  varintfileh = open("tot"+output+".avg",'w')
  vdwfileh.write("AVG fp\n")
  elefileh.write("AVG fp\n")
  intfileh.write("AVG fp\n")
  varvdwfileh.write("VAR fp\n")
  varelefileh.write("VAR fp\n")
  varintfileh.write("VAR fp\n")

  for j in range(len(int_list1)):
    for k in range(len(int_list2)):
      avg_mat_int[j][k] = avg_mat_int[j][k]/num_frames
      avg_mat_vdw[j][k] = avg_mat_vdw[j][k]/num_frames
      avg_mat_ele[j][k] = avg_mat_ele[j][k]/num_frames

      avg2_mat_int[j][k] = avg2_mat_int[j][k]/num_frames
      avg2_mat_vdw[j][k] = avg2_mat_vdw[j][k]/num_frames
      avg2_mat_ele[j][k] = avg2_mat_ele[j][k]/num_frames

      var_mat_int[j][k] = avg2_mat_int[j][k] - avg_mat_int[j][k]**2.0 
      var_mat_vdw[j][k] = avg2_mat_vdw[j][k] - avg_mat_int[j][k]**2.0 
      var_mat_ele[j][k] = avg2_mat_ele[j][k] - avg_mat_int[j][k]**2.0 
      ...
      if k < (len(int_list2)-1): # print a comma
        ...
        varvdwfileh.write('%f,'%(var_mat_vdw[j][k]))
        varelefileh.write('%f,'%(var_mat_ele[j][k]))
        varintfileh.write('%f,'%(var_mat_int[j][k]))
      elif k == (len(int_list2)-1): # don't print a comma after the last colomn
        vdwfileh.write('%f'%(avg_mat_vdw[j][k]))
        elefileh.write('%f'%(avg_mat_ele[j][k]))
        intfileh.write('%f'%(avg_mat_int[j][k]))
