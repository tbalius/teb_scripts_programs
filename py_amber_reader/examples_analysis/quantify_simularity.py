...
## mod by Trent Balius at FNLRC, March 2023
...
def main():
  if len(sys.argv) != 8: # if no input
     print "syntax:  matrix csv, matrix csv, heatmap_threshold heatmap_min heatmap_max label1_filename label2_filename"
  ...
  heatmap_threshold  = float(sys.argv[3])
  ...
  mo1 = readmatrix(file1handel)
  mo2 = readmatrix(file2handel)
  file1handel.close()
  file2handel.close()
  xl = len(mo1)
  yl = len(mo1[0])
  ...
  m1    = numpy.zeros([xl,yl]) # original matrix
  m2    = numpy.zeros([xl,yl]) # Transposed matrix
  mdiff = numpy.zeros([xl,yl]) # difference matrix ori minus transposd. 
  sum2_1    = 0.0
  sum2_2    = 0.0
  sum2_d12  = 0.0
  print("dim=(%d,%d)"%(xl,yl))
  for i in range(xl):
      for j in range(yl):
           m1[i,j]    = mo1[i][j]
           m2[i,j]    = mo2[i][j]
           mdiff[i,j] = m1[i,j]-m2[i,j]
           sum2_1     = sum2_1  + m1[i,j]**2
           sum2_2     = sum2_2  + m2[i,j]**2
           sum2_d12   = sum2_d12 + mdiff[i,j]**2
  norm_1 = math.sqrt(sum2_1)
  norm_2 = math.sqrt(sum2_2)
  norm_d = math.sqrt(sum2_d12)
  print("norm of diff matrix = %f"%norm_d)
  print("norm of matrix1 = %f"%norm_1)
  print("norm of matrix2 = %f"%norm_2)

  sum2_d12_n = 0
  for i in range(xl):
      for j in range(yl):
          normdiff = m1[i,j]/norm_1 - m2[i,j]/norm_2
          sum2_d12_n = sum2_d12_n + normdiff**2 

  norm_d_n = math.sqrt(sum2_d12_n) 
  print("normalized norm of diff matrix = %f"%(norm_d_n))
  txtfilehandel.close()
  write_mat(mdiff,file1name+'_sym_out.csv')
  print("### DIFF (Matrix1-Matrix2) ####")
  heatmap(mdiff,file1name+'_'+file2name+'_out',heatmap_threshold,vmin,vmax,lab1,lab2)
  return
