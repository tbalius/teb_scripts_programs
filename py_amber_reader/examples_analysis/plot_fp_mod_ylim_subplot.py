...
def plot_data(vec1,start1,stop1,start2,stop2,filename,lab1file, ylim_val):
...
def main():
  if len(sys.argv) != 6: # if no input
     ...
     print "syntax:  filename_prefix, label1_filename, y-axis_max_val reg1 reg2"
  ...
  #lab1               = sys.argv[5] 
  #lab2               = sys.argv[6] 
  start1 = int(reg1.split('-')[0])
  stop1 = int(reg1.split('-')[1])
  start2 = int(reg2.split('-')[0])
  stop2 = int(reg2.split('-')[1])
  ...
  file1handel.close()
  plot_data(m,start1,stop1,start2,stop2,file1name+'zoom.png',lab1,ylim_v)
  return
