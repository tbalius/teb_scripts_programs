...
def main():
  ...
  mo    = numpy.zeros([xl,yl]) # original matrix
  mT    = numpy.zeros([xl,yl]) # Transposed matrix
  ...
  for i in range(xl):
      for j in range(yl):
           mo[i,j] = m_t[i][j]
           mT[i,j] = m_t[j][i]
           mdiff[i,j]= mo[i,j]-mT[i,j]
           sum2_o = sum2_o + mo[i,j]**2
           sum2_s = sum2_s + mdiff[i,j]**2
  norm_o = math.sqrt(sum2_o)
  norm_s = math.sqrt(sum2_s)
  print("norm of sym matrix = %f"%norm_s)
  print("norm of ori matrix = %f"%norm_o)
  print("normalized norm of sym matrix = %f"%(norm_s/norm_o))
  txtfilehandel.close()
  write_mat(mdiff,file1name+'_sym_out.csv')
  print("### DIFF (ORIGINAL-TRANSPOSE) ####")
  heatmap(mdiff,file1name+'_sym_out',heatmap_threshold,vmin,vmax,lab1,lab2)
  print("### ORIGINAL ####")
  heatmap(mo,file1name+'_ori_out',heatmap_threshold,vmin,vmax,lab1,lab2)
  return
