

import sys
import copy

import math

import matplotlib
matplotlib.use('Agg')  # allows you to not have an x-server running

import scipy, pylab, numpy


## Writen by Trent Balius, 2019 
## modified to plot polar for dihiderals  (2021)

def read_rmsd_dat_file(filehandel):
    # si is the user specified i value
    # sj is the user specified j value
    rmsd = []
    time = []
    for line in filehandel:
        if "Frame" in line: 
            continue
        splitline = line.strip().split()
        time.append(float(splitline[0]))
        rmsd.append(float(splitline[1]))
    return rmsd, time



#def heatmap(Mat,label,filename,threshold,heatmap_threshold):
def make_plot(val_array,time_array,title,filename):

     print "Here in make_plot"
     fig = pylab.figure(figsize=(8,8))

     val_array_rad = []
     # dihideral are from -180 to 180 and we want them to be from 0 to 360. 
     for i in range(len(val_array)):
         val_array_rad.append(0.0)
         #if val_array[i] < 0:
         #   #print val_array[i]
         #   val_array[i] = 360 + val_array[i]
         ## pi = 180
         val_array_rad[i] = math.pi * val_array[i] / 180
         #print val_array[i]

     ax1 = fig.add_axes([0.1,0.1,0.6,0.2],projection='polar')
     #matplotlib.pyplot.plot(time_array,val_array,'k-') # draws a datshed line where dendogram is cut.
     #ax1.plot(val_array[0:100],'k.') # draws a datshed line where dendogram is cut.
     ax1.plot(val_array_rad,time_array,'k.') # draws a datshed line where dendogram is cut.
     #ax1.plot(time_array[0:100],val_array[0:100],'k.') # draws a datshed line where dendogram is cut.
     #matplotlib.pyplot.plot(val_array,time_array,'k.') # draws a datshed line where dendogram is cut.
     #ax1.set_xlim([-180,180])
     #ax1.set_ylim([-180,180])
     #ax1.set_title(title)
        
     ax2 = fig.add_axes([0.75,0.1,0.2,0.2])
     #n1_1, bins, patches = ax2.hist(val_array, normed=1, histtype='bar',orientation="horizontal")
     n1_1, bins, patches = ax2.hist(val_array, normed=1, histtype='bar')
     ax2.set_yticks([])
     #fig.show()
     fig.savefig(filename+'.png',dpi=600)   
     return


def main():
  if len(sys.argv) != 4: # if no input
     print "syntax:  dihideral dat file, title of plot, output prefix"
     print "Error:  you have entered the wrong number of inputs:"
     print len(sys.argv)
     exit()

  print "You have entered in %d inputs:"%(len(sys.argv))
  file1name  = sys.argv[1] 
  title     = sys.argv[2] 
  output    = sys.argv[3] 

  #print "input matrix file prefix = " + file1name+'.'+str(i)
  file1handel = open(file1name,'r')
  rv,tv = read_rmsd_dat_file(file1handel)
  file1handel.close()
  make_plot(rv,tv,title,output)
  return
 
main()

